"""
Puzzle backends for the puzzle: "Please Prove You Are Human".
"""

import json, logging, itertools, random
...
class PuzzleRequest:
    submittedImages = {'selected':[],'unselected':[]}
    images = []
    success = False
    operation = None
    errMsg = ""
    ...
    def __init__(self, payload):
        ...
        self.key = payload['key'] or self.get_hash_from_index(1)
        self.operation = int(payload['op'])
        self.category = self.get_category_from_key()
        if self.operation == 1:
            self.images = self.get_images()
        if self.operation == 2:
            self.submittedImages = {
                'selected': [ i for i in payload['selected'].split("|") if i],
                'unselected': [ i for i in payload['unselected'].split("|") if i]
            }
            self.validate_submission()

    def get_category_from_key(self) -> str:
        """The puzzle will submit a hash as part of its payload; retrieve the matching category from this"""
        if not self.key:
            return self.get_hash_from_index(1)
        try:
            return [i[2] for i in IMAGE_CATEGORIES if i[1] == self.key][0]
        except IndexError:
            return self.get_hash_from_index(1)

    def get_hash_from_index(self, idx) -> str:
        '''Return a category, given an index'''
        try:
            return [i[1] for i in IMAGE_CATEGORIES if i[0] == idx][0]
        except IndexError:
            return ""

    def get_next_category_from_key(self) -> str:
        try:
            return [i[3] for i in IMAGE_CATEGORIES if i[1] == self.key][0]
        except IndexError:
            return ""
    ...
    def get_images(self) -> list:
        ...
        # add herrings
        category_herrings = [ k for k, v in IMAGE_DATA[self.category].items() if v[2] == 1]
        images.extend(random.sample(category_herrings, k=3))
        # add true positives
        correct_images = random.randrange(3,5)
        category_true_positive = [ k for k, v in IMAGE_DATA[self.category].items() if v[0] == 1 and k not in images ]
        images.extend(random.sample(category_true_positive, k=correct_images))
        # add incorrect images
        category_incorrect = [ k for k, v in IMAGE_DATA[self.category].items() if v[1] == 1 and k not in images ]
        images.extend(random.sample(category_incorrect, IMAGES_REQUIRED - len(images)))
        random.shuffle(images)
        return images

    def validate_submission(self) -> list:
        images = [i for i in self.submittedImages['selected'] if i ]
        images.extend([ i for i in self.submittedImages['unselected'] if i ])
        images = list(set(images))
        if not len(images) == IMAGES_REQUIRED:
            self.errMsg = "Please try again"
            return

        if len(self.submittedImages['selected']) == 0:
            self.errMsg = "Please try again"
            return

        # check all selected images are valid
        submitted_are_valid = all([x in [k for k, v in IMAGE_DATA[self.category].items() if v[0] == 1] for x in self.submittedImages['selected']])
        if not submitted_are_valid:
            self.errMsg = "Please try again"
            self.debug = "not all submitted are valid"
            return

        # check that no unselected images are valid
        unsubmitted_are_valid = any([x in [k for k, v in IMAGE_DATA[self.category].items() if v[0] == 1] for x in self.submittedImages['unselected']])
        if unsubmitted_are_valid:
            self.errMsg = "Please try again"
            self.debug = "some unsubmitted are valid"
            return

        self.debug = "successful orig_key: " + self.payload['key'] + " key: " + self.key
        self.success = True
    ...
    def return_response(self):
        if self.operation == 1:
            ...
            return JsonResponse([
                ...
                self.errMsg,
                self.get_next_category_from_key() if self.success and self.operation == 2 else self.key,
                # self.debug
            ],
            safe=False)
        else:
...
def state_view(request: HttpRequest):
    # try:
        ...
        # responseData = {}
        # logger.exception(json.dumps(request.body))

        # statuses = []
    # logger.warn(json.dumps(request.body))
    return PuzzleRequest(request.POST).return_response()

    # except Exception as e:
    #     logger.exception("Unhandled exception in p216")
